		...
		?>
          ...
          <td>
			<table width="192" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td height="152" bgcolor="#CCCCCC" align="center"><a href="<?=$link_page?>" ><img src="<?=$board_img?>" width="190" height="150" /></a></td>
              </tr>
              <tr>
                <td height="46" valign="bottom"><div align="center"><input type="checkbox" name="chklist" class=border value="<?=$result[$i][uid]?>"><a href="<?=$link_page?>"  class="L2"><?=$row_board_mode?><?=$common->cut_string($result[$i][title],12)?></a> <?=$result_memo_count?></a><br />
                    <font class="T8"><?=$row_branch[cname]?> | <?=strtr(substr($result[$i][reg_date],0,10),"-",".")?></font></div></td>
              </tr>
            </table>
		  </td>
...
<!-- ================ 리스트 형태 출력인 경우 ======================= -->
	...
	<form name="listform" method="post"> 	
	  <table width="880" border="0" cellspacing="0" cellpadding="0">
		...
		<?}?>
          ...
          <td align="center" background="img/admin_14.gif"><font  class="T4">상품명</font></td>
	...
	?>
		...
		<?}?>
          <td>&nbsp;<a href="<?=$link_page?>" class="L4"><?=$row_board_mode?><?=$result[$i][title]?></a> <?=$row_reply?> <?=$result_memo_count?></td>
        ...
        <tr>
          ...
          <td width="98%" valign="top">
			<table width="517" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
				<td width=100>
					<select name="find_mode" class="FORM1">
					  <option value="">-분류-</option>
					<? for($j=1;$j<=count($ARR_BOARD_TYPE);$i++,$j++) { ?>
						<option value="<?=$j?>" <?if($get_mode=="$j") echo"selected";?>><?=$ARR_BOARD_TYPE[$j]?></option>
					<?}?>
					</select>
				</td>
				<td width=100>
                ...
                <td width="67" height="42">
					<select name="find_field" class="FORM3">
						<option value="title" <?=($get_field=="title"  )?"selected":"";?> >상품명</option>
