...
if($conf) {
    ...
    switch( $formmode ){        
		...
		########### 저장하기 처리 ##################		
			...
			$tran_query[0] = "INSERT INTO $tablename (uid,title,moveurl,reg_date) VALUES ('$new_uid','$title','$moveurl','$reg_date')";
		...
		########### 수정 하기 처리 ##################		
			...
			$tran_query[0] = "UPDATE $tablename SET title='$title',reg_date='$reg_date',moveurl='$moveurl'  WHERE uid='$uid'";
			$tran_result = $db->tran_query( $tran_query );


			if ( $tran_result == "1" ) {
				$common->error("수정 되었습니다.  ","goto_no_alert","$_SERVER[PHP_SELF]?bmain=write&conf=$conf");
			} else {
				$common->error("등록 실패 되었습니다","previous","");
			}			
