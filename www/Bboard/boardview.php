...
?>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
   ...
   <tr>
	...
	<td width="130" class="qna_view02"><span class="b c333">작성일</span>&nbsp;&nbsp;&nbsp;<?=$common->dateStyle(substr($row_board[reg_date],0,10),".")?></td>
	<td width="80" class="qna_view02"><span class="b c333">조회</span>&nbsp;&nbsp;&nbsp;<?=$row_board[ref]?></td>
	</tr>
   <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
     <tr>
      <td align="center" style="padding:20px 0 50px 0">
			<a href="<?echo"$_SERVER[PHP_SELF]?bmain=list"?>"><img src="../img/button/list.gif" hspace="2"></a>
			<?if($MEMBER_WRITE=="Y") {?>
				<a href="<?echo"$_SERVER[PHP_SELF]?bmain=write"?>"><img src="../img/button/write.gif" alt="글쓰기"></a>
			<?}?>
			<?if($SITE_USER_MID==$row_board[mid]) {?>	
				<a href="<?echo"$_SERVER[PHP_SELF]?bmain=write&uid=$uid"?>"><img src="../img/button/modify.gif" alt="수정" hspace="2"></a>
			<?}?>
			<?if($MEMBER_REPLY=="Y") {?>
				<a href="<?echo"$_SERVER[PHP_SELF]?bmain=reply&uid=$uid"?>" ><img src="../img/button/replay.gif" alt="답변" hspace="2"></a>
			<?}?>

			<?if(!$SITE_USER_MID) {?>
				<?if($MEMBER_WRITE=="Y") {?>
					<a href="<?echo"$_SERVER[PHP_SELF]?bmain=delete&uid=$uid"?>"><img src="../img/button/delete.gif" alt="삭제" hspace="2"></a>
				<?}?>
			<?} else {?>
				<?if($SITE_USER_MID==$row_board[mid]) {?>	
					<a href="javascript: onClick=contentDel('<?echo"$_SERVER[PHP_SELF]?bmain=ok&conf=$conf&uid=$uid&formmode=delete"?>');" ><img src="../img/button/delete.gif" alt="삭제" hspace="2"></a>
				<?}?>
			<?}?>
     ...
     <tr>
      <td valign="top" style="padding:0 0 50px 0"><table width="100%" border="0" cellspacing="0" cellpadding="0">
       <tr>
        ...
        <td class="qna_view_list01"><a href="<?if($back_next[backuid]) {?><?echo"$_SERVER[PHP_SELF]?bmain=view&uid=$back_next[backuid]"?><?} else {?>#<?}?>" ><?=$back_next[backtitle]?></a></td>
       </tr>
       <tr>
        <td width="60" align="center" class="qna_view_list02"><span class="b c333">다음</span></td>
        <td class="qna_view_list02"><a href="<?if($back_next[nextuid]) {?><?echo"$_SERVER[PHP_SELF]?bmain=view&uid=$back_next[nextuid]"?><?} else {?>#<?}?>"><?=$back_next[nexttitle]?></a></td>
       </tr>
    </table>
	</td></tr></table>