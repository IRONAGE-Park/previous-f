<?php include "../inc/header.php"; ?>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 <tr>
  <td height="480">
<!--visual-->
   <div class="mainBanner">
    <div class="mainSlide">
      <a href="#" style="cursor:default"><img src="../img/main/img1.jpg" alt="1"/></a>
      <a href="#" style="cursor:default"><img src="../img/main/img2.jpg" alt="2"/></a>
      <a href="#" style="cursor:default"><img src="../img/main/img3.jpg" alt="3"/></a>
     </div>
      <a href="#" class="btnPre"><img src="../img/main/visual_prev.png" alt="prev" /></a>
      <a href="#" class="btnNext"><img src="../img/main/visual_next.png" alt="next" /></a>
     <div class="paging">
      <div class="paging_in">
       <a href="#" class="active"></a> 
       <a href="#"></a> 
       <a href="#"></a>
      </div>          
     </div>
    </div>
<!--//visual-->
  </td>
 </tr>
<!--new products-->
 <tr>
  <td class="newp" align="center">
	<table width="1100" border="0" cellspacing="0" cellpadding="0">
   <colgroup>
    <col width="204"/><col width="20"/><col width="204"/><col width="20"/><col width="204"/><col width="20"/><col width="204"/><col width="20"/><col width="204"/><col width="20"/><col width="204"/>
   </colgroup>
   <tr>
    <td colspan="9" style="padding:0 0 40px 0" align="left"><img src="../img/main/text01.png"></td>
    </tr>
   <tr>
    <td valign="top"><?php include "list01.php"; ?></td>
    <td></td>
    <td valign="top"><?php include "list01.php"; ?></td>
    <td></td>
    <td valign="top"><?php include "list01.php"; ?></td>
    <td></td>
    <td valign="top"><?php include "list01.php"; ?></td>
    <td></td>
    <td valign="top"><?php include "list01.php"; ?></td>
   </tr>
   <tr>
    <td valign="top"><?php include "list01.php"; ?></td>
    <td></td>
    <td valign="top"><?php include "list01.php"; ?></td>
    <td></td>
    <td valign="top"><?php include "list01.php"; ?></td>
    <td></td>
    <td valign="top"><?php include "list01.php"; ?></td>
    <td></td>
    <td valign="top"><?php include "list01.php"; ?></td>
   </tr>
  </table></td>
 </tr>
<!--//new products-->
<!--portfolio-->
 <tr>
  <td class="port" align="center"><table width="1100" border="0" cellspacing="0" cellpadding="0">
   <tr>
    <td rowspan="2" align="left" valign="top">
		<table border="0" cellspacing="0" cellpadding="0">
     <colgroup>
      <col width="234"/><col width="234"/><col width=""/>
     </colgroup>
     <tr>
      <td colspan="3" style="padding:0 0 10px 0"><a href="javascript:go_09_01()"><img src="../img/main/text02.png" alt="Portfolio"></a></td>
     </tr>
     <tr>
      <td align="left" valign="top"><?php include "list02.php"; ?></td>
      <td align="left" valign="top"><?php include "list02.php"; ?></td>
      <td align="left" valign="top"><?php include "list02.php"; ?></td>
     </tr>
    </table></td>
    <td width="330" align="left" valign="top">
		<table width="330" border="0" cellspacing="0" cellpadding="0">
     <colgroup>
      <col width="242"/><col width="88"/>
     </colgroup>
     <tr>
      <td colspan="2" style="padding:0 0 10px 0"><a href="javascript:go_09_02()"><img src="../img/main/text03.png" alt="QnA"></a></td>
      </tr>
     <tr>
      <td><a href="#" class="main03">&middot; 견적에 포함된 공사내용은 어...</a></td>
      <td align="right" class="f14 c666">2014-00-00</td>
     </tr>
     <tr>
      <td><a href="#" class="main03">&middot; 견적에 포함된 공사내용은 어...</a></td>
      <td align="right" class="f14 c666">2014-00-00</td>
     </tr>
     <tr>
      <td><a href="#" class="main03">&middot; 견적에 포함된 공사내용은 어...</a></td>
      <td align="right" class="f14 c666">2014-00-00</td>
     </tr>
    </table></td>
   </tr>
   <tr>
    <td style="padding:50px 0 0 0"><img src="../img/main/text04.png"></td>
   </tr>
  </table></td>
 </tr>
<!--//portfolio-->
</table>
<?php include "../inc/footer.php"; ?>
